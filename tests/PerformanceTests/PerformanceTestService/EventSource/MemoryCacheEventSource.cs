...
namespace PerformanceTestService
{
    ...
    internal sealed class MemoryCacheEventSource : System.Diagnostics.Tracing.EventSource
    {
        ...
        protected override void OnEventCommand(EventCommandEventArgs command)
        {
            ...
            {
                _cacheItemCounter ??= new PollingCounter(CacheItemCounterName, this, () => Volatile.Read(ref _totalItemCount))
                {
                    DisplayName = "Total cache items (Write - Remove)"
                };
                _cacheWriteCounter ??= new PollingCounter(CacheWriteCounterName, this, () => Volatile.Read(ref _writeCount))
                {
                    DisplayName = "Total cache write calls"
                };
                _cacheReadCounter ??= new PollingCounter(CacheReadCounterName, this, () => Volatile.Read(ref _readCount))
                {
                    DisplayName = "Total cache read calls"
                };
                _cacheReadMissCounter ??= new PollingCounter(CacheReadMissCounterName, this, () => Volatile.Read(ref _readMissCount))
                {
                    DisplayName = "Total cache read misses"
                };
                _cacheRemoveCounter ??= new PollingCounter(CacheRemoveCounterName, this, () => Volatile.Read(ref _removeCount))
                {
                    DisplayName = "Total cache remove calls"
                };
                _cacheSizeCounter ??= new PollingCounter(CacheSizeCounterName, this, () => Volatile.Read(ref _cacheSizeInBytes))
                {
